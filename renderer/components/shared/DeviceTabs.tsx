...
// remote-awareness of its own. The tabs are the pill the worktree
// header marks a device with (its connection on the dot; this device
// has none to report), the picked one in the accent fill every
// selection in the app wears; the rest stay the chip's quiet card
// pill, which doubutsu fills through the data-slot once the hairline
// is gone. One row that scrolls sideways when the devices outnumber
// the width, never wrapping, so the title row below keeps its place
// however many machines there are. Left and right arrows move the
// pick, as tabs do.
...
import { HostScopeProvider, type HostApi } from "@/hooks/remote/useHostScope";
...
// Every device on the account as a tab, for the pages that span a
// whole machine rather than one repo: this device first (a hostless
// client has none), then the reachable peers, then the rest, so the
// machines that can answer sit where the eye starts. A peer is granted
// while its verdict is still in flight, the sidebar's rule, rather
// than flashing a refusal that turns into a body a moment later.
export function useAccountDeviceTabs(): DeviceTab[] {
...
export function DeviceTabBar({
  ...
  // The caller's bleed: the row scrolls out under the header's inset
  // rather than clipping at it, so it carries the header's padding as
  // negative margin plus padding of its own.
  className?: string;
}) {
  ...
  // which would also pull every scrolling ancestor), by its own inset.
  useEffect(() => {
  ...
  return (
    <div
      ...
      aria-label="Device"
      className={cn(
        "flex [scrollbar-width:none] gap-1.5 overflow-x-auto",
        className,
      )}
    >
      {tabs.map((tab) => {
...
// possessive ("its copy of this project", "its forest"), for both
// sentences.
export function DeviceTabPanel({
  ...
  if (tab.isThisDevice) return children;
  if (tab.api === undefined || tab.block === "offline") {
    return (
      <div className="p-6">
        <EmptyPanel>
          {tab.label} is offline. {capitalize(subject)} loads when it
          reconnects.
        </EmptyPanel>
      </div>
    );
  }
  if (tab.block === "no-grant") {
    return (
      <div className="p-6">
        <EmptyPanel>
          {tab.label} doesn&apos;t allow control from other devices yet. Allow
          it on its Devices page to work on {subject} here.
        </EmptyPanel>
      </div>
    );
  }
  // Remounted per device: a body seeds from the picked device's own
