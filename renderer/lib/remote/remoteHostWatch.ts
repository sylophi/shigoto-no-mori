...
// project's branch state. Both ride the peer's direct session and
...
// sweep (remoteDeviceSync's noteConvergence) covers whatever changed
// while a session was down, so this subscription never needs to know a
// session's lifecycle.
//
// Deliberately NOT mirrored, so this stays two channels:
...
export function startRemoteHostWatch(queryClient: QueryClient): void {
  window.api.hub.onPeerPush(({ deviceId, channel, payload }) => {
    ...
    if (channel === REFS_REFRESHED.channel) {
      // The bridge forwards pushes wholesale, so the payload is parsed
