// AccountRelay: one Durable Object per account (named by accountId),
...
// so there is no account check on the relay path because there is
...
export class AccountRelay implements DurableObject {
  ...
  // throws on send must not tear down the caller's socket. On the relay
