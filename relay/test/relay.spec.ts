...
// runs against the real AccountRelay Durable Object under workerd.
...
describe("GET /connect", () => {
  ...
  it("rejects a structurally malformed ticket before the upgrade", async () => {
    const response = await call(
      new Request(
        `${BASE}${RELAY_ROUTES.connect.path}?${CONNECT_TICKET_PARAM}=garbage`,
...
describe("relaying", () => {
  it("relays an opaque frame between two devices, byte for byte", async () => {
    ...
    // The relay direction is symmetric.
