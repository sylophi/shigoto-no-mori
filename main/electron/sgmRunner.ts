// Spawns the bundled sgm CLI as the app's worktree engine. The five
...
// packaged, dist-cli/sgm-d in dev (built by `pnpm dev`) -- so no PATH
...
// One NDJSON document from sgm's --json stream. `event` is set on
...
export function sgmBinaryPath(): string | null {
...
export function sgmAvailable(): boolean {
  return !isWindows && sgmBinaryPath() !== null;
}

const children = new Set<ChildProcess>();

export function sgmChildCount(): number {
  return children.size;
}

// Quit-time reap, mirroring killAllScripts for the TS engine: an sgm
...
export function killAllSgm(): void {
...
// Runs `sgm --json <args>`, parsing each stdout line as a document and
...
export function runSgm(
  args: string[],
  onDoc?: (doc: SgmDoc) => void,
): Promise<SgmResult> {
  const binary = sgmBinaryPath();
  if (binary === null) {
    return Promise.reject(
      new Error(
        "The sgm CLI binary is missing; run `pnpm cli:build --dev` (dev) or reinstall the app.",
      ),
    );
  }
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk: Buffer) => {
      ...
      for (
        ...
        if (!line) continue;
        try {
          const doc = JSON.parse(line) as SgmDoc;
          docs.push(doc);
          onDoc?.(doc);
        } catch {
          console.warn("[sgm] unparseable output line:", line.slice(0, 200));
        }
      }
    });
