...
export async function relocateWorktreeToManagedPath(
  ...
  // Same tombstone protocol as deletes: a quit mid-`git worktree move`
  ...
  return withDeleteInflight(
    ...
    async () => {
      ...
      // Everything we need for the moved identity is already known: the id
      // is path-derived, branch/detached survive the move, and we just
      // moved it into a managed prefix the user picked. Skipping the
      // post-move `git worktree list` keeps the relocate batch fast.
