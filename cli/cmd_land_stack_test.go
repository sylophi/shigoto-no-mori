...
// chain (#2 layer-a, merged; #3 layer-b and #4 layer-c, open), no
...
case "$*" in
  ...
  "pr list --state all --head layer-b --limit 1 --json "*) echo '[{"number":3,"title":"B","state":"OPEN","isDraft":false,"url":"u3","baseRefName":"layer-a","headRefName":"layer-b"}]';;
...
func TestStackedUnder(t *testing.T) {
	...
	below, err := stackedUnder(proj, &top)
	...
	if below, err := stackedUnder(proj, &bottom); err != nil || below != nil {
