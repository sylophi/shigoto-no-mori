...
// sm cd / sm worktree (wt) drops into a worktree. A child process
...
// directive file lets the calling shell cd instead, with no nesting.
...
func cmdEnterWorktree(ctx cliContext, args []string, acrossProjects bool) (int, error) {
	...
	// (`sm cd root` from the primary) can still name it. A nested shell
	// in the same directory helps nobody.
