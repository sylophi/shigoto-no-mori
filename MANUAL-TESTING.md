...
| `window.api.mirror.start({...})`                                    | Brings a peer's worktree here and keeps the two mirrored (files both ways, git state followed). The `sync.pullWorktree` payload plus `ignoreMode` (`everything`, `gitignored`, `custom`, `bring`) and `ignores` (engine patterns, `/path` anchors to the root). `bring` is gitignored with exceptions: the gitignore rules, then a `!/path`, `!/path/**` pair per ignored path that crosses anyway (`bringIgnores` in `shared/ipc/modules/sync.ts`). `runSetup: false` on either pull skips the setup script at the create (the dialogs default it off when nothing is left out, and on once the rule leaves something behind). Returns the local worktree and the `session`. |
...
| `window.api.mirror.stop(session)` / `pause` / `resume`              | Controls a session this device runs. Stop also removes the copy here (a forced delete, and the source keeps its own), so the page moves on.                                                                                                                                                                                                                                                                 |
