...
| `window.api.mirror.start({...})`                                    | Brings a peer's worktree here and keeps the two mirrored (files both ways, git state followed). The `sync.pullWorktree` payload plus `ignoreMode` (`everything`, `gitignored`, `custom`) and `ignores` (engine patterns, `/path` anchors to the root). `runSetup: false` on either pull skips the setup script at the create (the dialogs default it on for a transplant and for a mirror that leaves nothing out). Returns the local worktree and the `session`. |
