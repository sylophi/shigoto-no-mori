...
// SAME stub Durable Object as check-relay-link.mjs
// (scripts/lib/relayStub.mjs) and drives the web connection as device A
// against a real node HOST peer (device B, host/relay/connection.ts,
// booted through the shared relayBoot fixture) answering the broker
// slot.
//
...
// the ONLY channel the orchestration-only relay wire serves, and the
...
import { CLOSE_DEVICE_REVOKED, CLOSE_SUPERSEDED } from "@shared/relay/protocol";
import { createRelayConnection as createWebConnection } from "../web/relay/connection.ts";
import { makeProof } from "./lib/checkKit.mjs";
import { bootDevice as bootHost, delay, waitFor } from "./lib/relayBoot.mjs";
import { startStubRelay } from "./lib/relayStub.mjs";

// The broker slot is the only thing the relay wire serves, so the host
...
async function main() {
  ...
  await check(
    "invoke: the broker channel round-trips through the relay",
    async (track) => {
      const stub = await startStubRelay();
      track(() => stub.close());
      const a = await bootWeb(stub, "A", {}, track);
