...
check("crash loop allows a burst up to the ceiling", () => {
  let recent = [];
  let now = 1_000;
  for (let i = 0; i < CRASH_LOOP_MAX; i++) {
    const decision = shouldRecreateWindow(recent, now);
    assert.equal(decision.recreate, true, `recreation ${i + 1} should proceed`);
    recent = decision.recent;
    now += 1_000;
  }
  assert.equal(recent.length, CRASH_LOOP_MAX);
});

check("crash loop gives up past the ceiling", () => {
  // CRASH_LOOP_MAX recreations already recorded inside the window.
  const recent = Array.from({ length: CRASH_LOOP_MAX }, (_, i) => 1_000 + i);
  const decision = shouldRecreateWindow(recent, 2_000);
  assert.equal(decision.recreate, false, "the extra crash must give up");
  // Giving up records nothing new: the list is only pruned, not grown.
  assert.equal(decision.recent.length, CRASH_LOOP_MAX);
});
...
check("crash loop resets once the window elapses", () => {
  ...
  const now = recent[recent.length - 1] + CRASH_LOOP_WINDOW_MS + 1;
  const decision = shouldRecreateWindow(recent, now);
  assert.equal(decision.recreate, true, "a crash after the window recovers");
...
check("fatal relaunch allows up to the cap", () => {
  let recent = [];
  let now = 5_000;
  for (let i = 0; i < FATAL_RELAUNCH_MAX; i++) {
    const decision = shouldRelaunchAfterFatal(recent, now);
    assert.equal(decision.relaunch, true, `relaunch ${i + 1} should proceed`);
    recent = decision.recent;
    now += 10_000;
  }
  assert.equal(recent.length, FATAL_RELAUNCH_MAX);
});

check("fatal relaunch stops past the cap", () => {
  const recent = Array.from(
    { length: FATAL_RELAUNCH_MAX },
    (_, i) => 5_000 + i,
  );
  const decision = shouldRelaunchAfterFatal(recent, 6_000);
  assert.equal(decision.relaunch, false, "a deterministic crash must stop");
  assert.equal(decision.recent.length, FATAL_RELAUNCH_MAX);
});
...
check("fatal relaunch resets once the window elapses", () => {
  ...
  const now = recent[recent.length - 1] + FATAL_RELAUNCH_WINDOW_MS + 1;
  const decision = shouldRelaunchAfterFatal(recent, now);
  assert.equal(decision.relaunch, true, "a relaunch after the window recovers");
  assert.deepEqual(decision.recent, [now], "stale timestamps are pruned");
});
...
check("the limiter constants are sane", () => {
  assert.ok(CRASH_LOOP_MAX > 0 && CRASH_LOOP_WINDOW_MS > 0);
  assert.ok(FATAL_RELAUNCH_MAX > 0 && FATAL_RELAUNCH_WINDOW_MS > 0);
});
