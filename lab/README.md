...
- **Desktop shell** — `pnpm exec vite --config vite.lab.config.ts`
  (port 5191). Mounts the desktop renderer (`renderer/App.tsx`); the
  lab page poses as "Studio Mac" with a local forest.
...
- `?peers=sm:connected,tp:connected,mini:online,pc:offline` — presence
  ...
  Work PC). Desktop default `tp:connected`; web default adds `sm`.
...
that has it installed) — `node shoot.mjs shots.json outdir`, with
