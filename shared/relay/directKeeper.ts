...
// only unsupervised one (the relay socket and cloudflared are both
...
// restarted, or our own relay link came back, both of which reset the
...
export type DirectKeeper = {
  // Feed the desired set: the live roster on every relay transition,
  // and [] whenever our own relay link is down (no roster, no
  // verdicts, and nothing to dial: the broker leg rides the relay).
    ...
    reconcile(online) {
      ...
      for (const [deviceId, state] of states) {
        if (!live.has(deviceId)) {
          ...
          // gate that keeps sessions alive through OUR OWN relay
      ...
      for (const deviceId of live) {
        ...
        // this a no-op resolve for a session that survived a relay
