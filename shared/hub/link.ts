...
// The hub socket itself is gone (torn down, or never up). Peer calls
// reject with this so callers can distinguish "my own hub socket is
...
export type HubLinkDeps = {
  ...
  // The one broker slot this wire serves (see HubBroker).
  broker: HubBroker;
...
type ClientPeer = {
  ...
  helloWaiter: {
    resolve: (session: HubBrokerSession) => void;
    reject: (error: unknown) => void;
...
export function createHubLink(deps: HubLinkDeps): HubLink {
  ...
  function hubFrameTexts(
    to: string,
    frame: ClientFrame | ServerFrame,
    epoch: number,
  ): { sendText: string; fits: boolean } {
    const hubFrame: HubFrame = { epoch, sm: frame };
    const sendText = encodeEnvelope({ t: "relay", to, frame: hubFrame });
    return { sendText, fits: hubTextWithinLimit(sendText, routingDelta(to)) };
  }
  ...
  function closeClientPeer(peer: ClientPeer): void {
    ...
    destroyClientPeer(peer, new HubLinkDownError());
  }
  ...
  function dropHostSession(deviceId: string): void {
    ...
    // hostile hub that flaps a peer's presence would otherwise zero
  ...
  function handleHello(from: string, epoch: number): void {
    ...
    const session: HostSession = {
      epoch,
      controller,
      // The minimal broker context (see HubBrokerContext): the
  ...
  async function dispatch(
    ...
    const encoded = hubFrameTexts(from, answer, session.epoch);
    if (!encoded.fits) {
      sendAnswer(
        from,
        {
  ...
  function connectBroker(deviceId: string): Promise<HubBrokerSession> {
    ...
    if (existing !== undefined) {
      destroyClientPeer(existing, new HubLinkDownError());
    }
    const epoch = nextEpoch++;

    return new Promise<HubBrokerSession>((resolve, reject) => {
      ...
      peer.helloWaiter = {
        ...
        timer: setTimeout(() => {
          ...
          destroyClientPeer(
            peer,
            new Error(`welcome timeout from hub peer ${deviceId}`),
          );
        }, HELLO_TIMEOUT_MS),
      };
  ...
  function applyPresence(list: readonly string[]): void {
    ...
    for (const [deviceId, peer] of clientPeers) {
      if (!online.has(deviceId)) {
        destroyClientPeer(peer, new HubPeerOfflineError(deviceId));
      }
    }
  ...
  function handleNack(to: string, reason: "offline" | "too-large"): void {
    ...
    if (reason === "offline") {
      ...
      if (peer !== undefined) {
        destroyClientPeer(peer, new HubPeerOfflineError(to));
      }
      dropHostSession(to);
      return;
    }
    ...
    if (peer !== undefined) {
      ...
      rejectAllPending(peer, new HubMessageTooLargeError());
    }
  }
  ...
  return {
    ...
    teardown(): void {
      const error = new HubLinkDownError();
