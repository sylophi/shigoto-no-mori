...
const runtimeDir = join(appRoot, ".electron-dev");
...
// Worktree-unique, human-scannable bundle id suffix: the worktree's
// directory name for the human, a hash of its path for uniqueness (two
// worktrees can share a basename across different parents). Both come
// from the worktree root, not app/: macOS keys a bundle's permissions
// to its id. Deliberately not
